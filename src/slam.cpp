...
int main(int argc, char **argv)
{
    ...
    std::string arg1 = argv[1];  // Get the first argument
    std::shared_ptr<rclcpp::Node> node;

    if (arg1 == "mono") {
        //pSLAM = new ORB_SLAM3::System(argv[2], argv[3], ORB_SLAM3::System::MONOCULAR, visualization);
        //node = std::make_shared<MonocularSlamNode>(pSLAM);
    }
    else
    if (arg1 == "rgbd") {
        printf("Hello  %s, %s", argv[2], argv[3]);
        pSLAM = new ORB_SLAM3::System(argv[2], argv[3], ORB_SLAM3::System::RGBD, visualization);
        printf("Hello2");
        node = std::make_shared<RgbdSlamNode>(pSLAM);
        
    }
    else 
    if (arg1 == "stereo") {
        //pSLAM = new ORB_SLAM3::System(argv[2], argv[3], ORB_SLAM3::System::STEREO, visualization);
        //node = std::make_shared<StereoSlamNode>(pSLAM, argv[3], argv[4]);
